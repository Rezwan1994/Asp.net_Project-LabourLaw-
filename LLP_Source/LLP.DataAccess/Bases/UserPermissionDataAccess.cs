...
namespace LLP.DataAccess
{
	public partial class UserPermissionDataAccess : BaseDataAccess
	{
		...
		{	
			AddParameter(cmd, pGuid(UserPermissionBase.Property_CompanyId, userPermissionObject.CompanyId));
			AddParameter(cmd, pGuid(UserPermissionBase.Property_UserId, userPermissionObject.UserId));
		...
		protected void FillObject(UserPermissionBase userPermissionObject, SqlDataReader reader, int start)
		{
				...
				userPermissionObject.CompanyId = reader.GetGuid( start + 1 );			
				userPermissionObject.UserId = reader.GetGuid( start + 2 );			
				if(!reader.IsDBNull(3)) userPermissionObject.PermissionId = reader.GetInt32( start + 3 );			
				if(!reader.IsDBNull(4)) userPermissionObject.PermissionGroupId = reader.GetInt32( start + 4 );			
			FillBaseObject(userPermissionObject, reader, (start + 5));
