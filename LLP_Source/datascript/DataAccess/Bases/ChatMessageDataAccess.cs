...
namespace LLP.DataAccess
{
	public partial class ChatMessageDataAccess : BaseDataAccess
	{
		...
		{	
			...
			AddParameter(cmd, pNVarChar(ChatMessageBase.Property_FileLocation, 500, chatMessageObject.FileLocation));
		}
		#endregion
		...
		protected void FillObject(ChatMessageBase chatMessageObject, SqlDataReader reader, int start)
		{
				...
				if(!reader.IsDBNull(6)) chatMessageObject.FileLocation = reader.GetString( start + 6 );			
			FillBaseObject(chatMessageObject, reader, (start + 7));
