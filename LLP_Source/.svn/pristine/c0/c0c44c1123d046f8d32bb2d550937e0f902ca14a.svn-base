...
namespace LLP.DataAccess
{
	public partial class PermissionDataAccess : BaseDataAccess
	{
		...
		protected void FillObject(PermissionBase permissionObject, SqlDataReader reader, int start)
		{
				...
				if(!reader.IsDBNull(2)) permissionObject.ParentId = reader.GetInt32( start + 2 );			
				if(!reader.IsDBNull(3)) permissionObject.Name = reader.GetString( start + 3 );			
				if(!reader.IsDBNull(4)) permissionObject.DisplayText = reader.GetString( start + 4 );			
				if(!reader.IsDBNull(5)) permissionObject.IsActive = reader.GetBoolean( start + 5 );			
