...
namespace LLP.DataAccess
{
	public partial class GlobalSettingDataAccess : BaseDataAccess
	{
		...
		protected void FillObject(GlobalSettingBase globalSettingObject, SqlDataReader reader, int start)
		{
				...
				if(!reader.IsDBNull(2)) globalSettingObject.SearchKey = reader.GetString( start + 2 );			
				if(!reader.IsDBNull(3)) globalSettingObject.Value = reader.GetString( start + 3 );			
				if(!reader.IsDBNull(4)) globalSettingObject.IsActive = reader.GetBoolean( start + 4 );			
